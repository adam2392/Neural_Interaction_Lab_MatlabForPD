% Input data from the excel sheet
% run analysis after outlier rejection

% 01:
%% PD No Medication Data
pdnomed_avgstep = [0.084932	0.10909	0.08842	0.10024	0.10604	0.095607	0.14699	0.16404	0.12472	0.11843	0.10377	0.15214	0.11691	0.10845	0.14427	0.13491	0.12871	0.10942	0.116];
pdnomed_varstep = [0.016863	0.012857	0.0224	0.020575	0.015754	0.0065991	0.019822	0.011313	0.017655	0.01747	0.0079	0.031761	0.020146	0.017638	0.019934	0.017644	0.016836	0.013687	0.017552];
pdnomed_avgvel = [0.59851	0.49214	1.047	0.5791	0.58862	0.28421	0.62388	0.36395	0.69356	0.98827	0.44177	0.73841	0.33201	0.24885	0.96057	0.85831	0.67309	0.64363	0.92673];
pdnomed_varvel = [2.0846	3.238	18.8498	3.9954	4.3037	0.55481	6.85	0.25942	0.3853	0.50011	0.31501	2.2834	0.15323	0.078149	24.9951	6.5652	5.0103	0.2831	12.9114];
pdnomed_totalsteps = [133	77	51	78	94	128	105	15	98	83	37	89	81	12	43	56	83	131	48];
pdnomed_avgcadence = [0.312	0.94461	2.1906	6.3833	1.1575	0.95054	0.70637	5.5078	1.9725	4.3027	3.1896	1.9252	0.65413	2.257	3.0626	0.80072	0.70653	0.13375	0.59386];
pdnomed_varcadence = [0.52688	1.9134	14.2487	166.1931	6.24	4.3123	0.81994	135.4322	6.6393	130.1182	24.7313	16.3057	0.92181	5.5084	30.0676	5.0417	1.6674	0.068452	0.85767];

%Display Mean and SD Stats for PD No Med
pdnomed_step = [nanmean(pdnomed_avgstep),std(pdnomed_avgstep)];
disp(pdnomed_step);
pdnomed_varstep = [nanmean(pdnomed_varstep) std(pdnomed_varstep)];
disp(pdnomed_varstep);
pdnomed_vel = [nanmean(pdnomed_avgvel) std(pdnomed_avgvel)];
disp(pdnomed_vel);
pdnomed_varvel = [nanmean(pdnomed_varvel) std(pdnomed_varvel)];
disp(pdnomed_varvel);
pdnomed_totalstep = [nanmean(pdnomed_totalsteps) std(pdnomed_totalsteps)];
disp(pdnomed_totalstep);
pdnomed_cad = [nanmean(pdnomed_avgcadence) std(pdnomed_avgcadence)];
disp(pdnomed_cad);
pdnomed_varcad = [nanmean(pdnomed_varcadence) std(pdnomed_varcadence)];
disp(pdnomed_varcad);

%% PD Medication Data

pdmed_avgstep = [0.10645	0.13498	0.098618	0.12003	0.094489	0.10596	0.10095	0.1101	0.2174	0.10989];
pdmed_varstep = [0.022631	0.014252	0.028109	0.017236	0.01816	0.015984	0.017397	0.019659	0.0219	0.0091764];
pdmed_avgvel = [0.71949	0.59647	0.8638	0.51242	0.66104	0.55109	0.85412	0.87129	1.1261	0.68375];
pdmed_varvel = [2.1688	2.8011	21.1159	6.189	7.8134	3.0613	15.0099	0.62857	0.49908	0.27488];
pdmed_totalsteps = [103	47	80	72	95	105	52	69	105	117];
pdmed_avgcadence = [1.1785	0.94032	0.49883	2.4764	0.44577	0.40749	0.80426	2.769	0.61983	2.8673];
pdmed_varcadence = [5.8598	2.263	0.92737	22.9482	2.2862	0.47862	1.7007	27.7814	4.5071	37.4319];

% Display Mean & SD of PD Med Data
pdmed_step = [nanmean(pdmed_avgstep),std(pdmed_avgstep)];
disp(pdmed_step);
pdmed_varstep = [nanmean(pdmed_varstep) std(pdmed_varstep)];
disp(pdmed_varstep);
pdmed_vel = [nanmean(pdmed_avgvel) std(pdmed_avgvel)];
disp(pdmed_vel);
pdmed_varvel = [nanmean(pdmed_varvel) std(pdmed_varvel)];
disp(pdmed_varvel);
pdmed_totalstep = [nanmean(pdmed_totalsteps) std(pdmed_totalsteps)];
disp(pdmed_totalstep);
pdmed_cad = [nanmean(pdmed_avgcadence) std(pdmed_avgcadence)];
disp(pdmed_cad);
pdmed_varcad = [nanmean(pdmed_varcadence) std(pdmed_varcadence)];
disp(pdmed_varcad);
...
control_step = [nanmean(control_avgstep),std(control_avgstep)];
disp(control_step);
control_varstep = [nanmean(control_varstep) std(control_varstep)];
disp(control_varstep);
control_vel = [nanmean(control_avgvel) std(control_avgvel)];
disp(control_vel);
control_varvel = [nanmean(control_varvel) std(control_varvel)];
disp(control_varvel);
control_totalstep = [nanmean(control_totalsteps) std(control_totalsteps)];
disp(control_totalstep);
control_cad = [nanmean(control_avgcadence) std(control_avgcadence)];
disp(control_cad);
control_varcad = [nanmean(control_varcadence) std(control_varcadence)];
disp(control_varcad);
