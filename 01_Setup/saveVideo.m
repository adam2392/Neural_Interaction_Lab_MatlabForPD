...
% Example: saveVideo('021.2', 'Color');

function saveVideo(subjNum, imageDir)
%% Initialize and find all image file names    
    %This depends on where you save your image folders **Subject to Change**
    subjexp = {'Walk', 'SitTurn', 'SitStand'};
    for ii=1:3
        expName = subjexp{ii};
        startDir = fullfile(strcat('/Volumes/NIL_PASS/Camera1/', subjNum, '_October28/', subjNum, '_', imageDir, '_', expName, '/'));
        outputDir = fullfile('/Volumes/NIL_PASS/Videos');
    ...
    %% Calculate Frame Rate of the Saved Images
        ...
        last = sortedImageNames{end}(timeStamp+3:timeStamp+11);
        first = sortedImageNames{1}(timeStamp+3:timeStamp+11);
    ...
    %% Save into a Video File Section
        ...
        outputName = strcat(subjNum,'_', expName,'_', first, 'until', last, '.avi');

        videoFile = fullfile(outputDir, outputName);
